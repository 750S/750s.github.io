  ...
  if(user){
    ...
    firebase.database().ref('Authorized Members').on('value', function(snapshot) {
      ...
      if(validUser){
          ...
          names=["Karthik","Nicholas","Tanav","Suchit","Jason","Suhruth","Abimanyu","Vijay","Tejas","Sanaa","Sharvani"];
          for(let z =0;z!=9;z++)
        ...
        firebase.auth().signOut();
        window.location.href = "https://750s.github.io/old";
      }
    });
  }else{
    console.log("SIGNED OUT");
    window.location.href = "https://750s.github.io/old"; //comment this to test site locally
  }
});
...
let indexToChange = 0;
function checkDate() {
  var meetingDate = new Date(new Date(dateInput.value).getTime()+18000001);  //brute forced to account for -4 GMT
